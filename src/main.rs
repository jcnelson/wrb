...
use crate::util::{DEFAULT_WRB_EPOCH, DEFAULT_WRB_CLARITY_VERSION};
...
fn load_wrbsite_source(wrbsite_name: &str, source: Option<String>) -> Result<Vec<u8>, String> {
    let Some(path) = source else {
        ...
        let bns_res = runner.bns_lookup(namespace, name)
            .map_err(|e| {
                format!("Failed to look up '{}': {:?}", wrbsite_name, &e)
            })?;

        let bns_rec = bns_res
            .map_err(|e| {
                format!("BNS error when looking up '{}': {:?}", wrbsite_name, &e)
            })?;

        // load as an attachment
        let attachment = runner.get_attachment(&bns_rec.zonefile_hash)
            .map_err(|e| {
                format!("Failed to get attachment for '{}' (hash {}): {:?}", wrbsite_name, &bns_rec.zonefile_hash, &e)
            })?;
    ...
    let bytes = Renderer::encode_bytes(code.as_bytes())
        .map_err(|e| {
            format!("Failed to encode source code from '{}': {:?}", &path, &e)
        })?;
...
fn inner_json_to_clarity(json_value: serde_json::Value) -> Result<Value, String> {
    match json_value {
        ...
        serde_json::Value::Number(num) => {
            if num.is_i64() {
                Ok(Value::Int(i128::from(num.as_i64().ok_or(format!("Not an i64: {:?}", &num))?)))
            }
            else if num.is_u64() {
                Ok(Value::UInt(u128::from(num.as_u64().ok_or(format!("Not an i64: {:?}", &num))?)))
            }
            else {
        ...
        serde_json::Value::String(s) => {
            let value = vm_execute(&s, DEFAULT_WRB_CLARITY_VERSION)
                .map_err(|e| {
                    format!("Could not execute string '{}': {:?}", &s, &e)
                })?
                .ok_or_else(|| {
                    format!("Failed to evaluate string '{}' to a value", &s)
                })?;
            Ok(value)
        }
        serde_json::Value::Array(value_vec) => {
            ...
            Ok(Value::cons_list(clarity_values, &DEFAULT_WRB_EPOCH).map_err(|e| format!("Could not build Clarity list: {:?}", &e))?)
        }
        serde_json::Value::Object(objs_map) => {
            let mut clarity_values = vec![];
            for (obj_name, obj_value) in objs_map.into_iter() {
                let clarity_name = ClarityName::try_from(obj_name.as_str()).map_err(|e| format!("Could not build Clarity tuple name: {:?}", &e))?;
                let clarity_val = inner_json_to_clarity(obj_value)?;
                clarity_values.push((clarity_name, clarity_val));
            }
            let tuple_data = TupleData::from_data(clarity_values).map_err(|e| format!("Could not build tuple from values: {:?}", &e))?;
...
fn json_to_clarity<R: Read>(fd: &mut R) -> Result<Value, String> {
    let json_obj : serde_json::Value = serde_json::from_reader(fd)
        .map_err(|e| {
...
fn subcommand_clarity(argv: Vec<String>) {
    ...
    if cmd == "encode-json" {
        ...
        println!("{}", clarity_val.serialize_to_hex()
            .map_err(|e| {
...
/// session ID
fn format_wrbpod_session(contract_addr: &QualifiedContractIdentifier, session_id: u128) -> Result<(), String> {
    ...
    let bns_contract_id = with_global_config(|cfg| cfg.get_bns_contract_id()).expect("FATAL: system not initialized");
    let mut runner = Runner::new(bns_contract_id, node_host, node_port);
    let home_stackerdb_client = get_home_stackerdb_client(&mut runner, contract_addr.clone(), privkey.clone())
        .map_err(|e| {
            format!("Failed to instantiate StackerDB client to {}: {:?}", contract_addr, &e)
        })?;
    
    let replica_stackerdb_client = get_replica_stackerdb_client(&mut runner, contract_addr.clone(), privkey.clone())
        .map_err(|e| {
            format!("Failed to instantiate StackerDB client to {}: {:?}", contract_addr, &e)
        })?;

    let wrbpod_session = Wrbpod::format(home_stackerdb_client, replica_stackerdb_client, privkey.clone())
        .map_err(|e| {
            format!("Failed to open wrbpod session to {}: {:?}", contract_addr, &e)
        })?;
...
fn setup_wrbpod_session(contract_addr: &QualifiedContractIdentifier, session_id: u128) -> Result<(), String> {
    // already instantiated?
    if with_globals(|globals| globals.get_wrbpod_session(session_id).is_some()) {
        return Ok(());
    }
    ...
    let wrbpod_session = Wrbpod::open(home_stackerdb_client, replica_stackerdb_client, privkey.clone())
        .map_err(|e| {
            format!("Failed to open wrbpod session to {}: {:?}", contract_addr, &e)
        })?;
...
fn get_wrbpod_contract_id(argv: &mut Vec<String>) -> Result<QualifiedContractIdentifier, String> {
    let contract_addr_str = if let Some(addr_str) = consume_arg(argv, &["-w", "--wrbpod"], true)
        ...
        let principal = privkey_to_principal(&privkey, if mainnet { C32_ADDRESS_VERSION_MAINNET_SINGLESIG } else { C32_ADDRESS_VERSION_TESTNET_SINGLESIG });
        let addr = StacksAddress { version: principal.0, bytes: Hash160(principal.1) };
        let addr_str = format!("{}.wrbpod", &addr);
        addr_str
    };
    let contract_addr = QualifiedContractIdentifier::parse(&contract_addr_str)
        .map_err(|_e| {
            format!("could not parse '{}'", &contract_addr_str)
        })?;
...
fn get_wrbpod_slot(contract_addr: &QualifiedContractIdentifier, app_name: &str, app_slot_num: u32) -> Result<Option<WrbpodSlices>, String> {
    ...
    let slot = with_globals(|globals| {
        ...
        wrbpod_session.fetch_chunk(&app_name, app_slot_num)
            .map_err(|e| {
...
fn subcommand_wrbpod(mut argv: Vec<String>, wrbsite_data_source_opt: Option<String>) {
    ...
    if cmd == "format" {
        if argv.len() < 3 {
            eprintln!("Usage: {} wrbpod {} [-w wrbpod_contract_addr]", &argv[0], &cmd);
            process::exit(1);
        }
        ...
        println!("{}", serde_json::to_string(&superblock).map_err(|e| {
        ...
        if argv.len() < 5 {
            eprintln!("Usage: {} wrbpod {} [-w wrbpod_contract_addr] [-s app_source] APP_NAME NUM_SLOTS", &argv[0], &cmd);
            process::exit(1);
        }
        let contract_addr = get_wrbpod_contract_id(&mut argv)
            .map_err(|e| {
        ...
        let num_slots : u32 = argv[4].parse()
            .map_err(|_e| {
        ...
        let res = with_globals(|globals| {
            let wrbpod_session = globals.get_wrbpod_session(0).unwrap();
            wrbpod_session.allocate_slots(&app_name, code_hash, num_slots)
                .map_err(|e| {
        ...
        let app_name = argv[3].clone();
        let app_slot_num : u32 = argv[4].parse()
            .map_err(|_e| {
        ...
        let slot : WrbpodSlices = if app_slot_path == "-" {
            ...
            let data = fs::read(&app_slot_path)
                .map_err(|e| {
                    eprintln!("FATAL: failed to read data from {}: {:?}", &app_slot_path, &e);
                    process::exit(1);
                })
                .unwrap();
            ...
            serde_json::from_slice(data.as_slice())
                .map_err(|e| {
                    eprintln!("FATAL: failed to decode data from JSON in {}: {:?}", &app_slot_path, &e);
                    process::exit(1);
                })
                .unwrap()
        }
        ...
        let Some(stackerdb_chunk_id) = with_globals(|globals| {
            ...
            eprintln!("FATAL: app slot {} is not occupied by app '{}'", app_slot_num, app_name);
            process::exit(1);
        };
        ...
        .map_err(|e| {
            eprintln!("FATAL: failed to list chunks in {}: {:?}", &contract_addr, &e);
            process::exit(1);
        })
        .unwrap();

        let slot_version = slots_metadata.get(stackerdb_chunk_id as usize)
            .map(|slot_md| slot_md.slot_version)
            .unwrap_or_else(|| {
                eprintln!("FATAL: no such StackerDB slot {} (app slot {}) in {}", stackerdb_chunk_id, app_slot_num, &contract_addr);
                process::exit(1);
            });
        ...
        with_globals(|globals| {
            let wrbpod_session = globals.get_wrbpod_session(0).unwrap();
            wrbpod_session.put_chunk(chunk_data)
                .map_err(|e| {
                    eprintln!("FATAL: failed to put app slot {} to '{}': {:?}", &app_slot_num, &app_name, &e);
                    process::exit(1);
                })
                .unwrap();
        });
        ...
        if argv.len() < 6 {
            eprintln!("Usage: {} wrbpod {} [-w wrbpod_contract_addr] APP_NAME APP_SLOT_NUM", &argv[0], &cmd);
            process::exit(1);
        }
        let contract_addr = get_wrbpod_contract_id(&mut argv)
            .map_err(|e| {
        ...
        if argv.len() < 4 {
            eprintln!("Usage: {} wrbpod {} [-w wrbpod_contract_addr] SLOT_NUM", &argv[0], &cmd);
            process::exit(1);
        }
        let contract_addr = get_wrbpod_contract_id(&mut argv)
            .map_err(|e| {
        ...
        let slot = with_globals(|globals| {
            let wrbpod_session = globals.get_wrbpod_session(0).unwrap();
            let chunk_data_opt = wrbpod_session.get_and_verify_raw_chunk(slot_num)
                .map_err(|e| {
                    eprintln!("FATAL: failed to fetch chunk: {:?}", &e);
                    process::exit(1);
                })
        ...
        if argv.len() < 5 {
            eprintln!("Usage: {} wrbpod get-slot [-w wrbpod_contract_addr] APP_NAME APP_SLOT_NUM", &argv[0]);
            process::exit(1);
        }
        let contract_addr = get_wrbpod_contract_id(&mut argv)
            .map_err(|e| {
        ...
        let slot = get_wrbpod_slot(&contract_addr, &app_name, app_slot_num)
            .map_err(|e| {
                eprintln!("FATAL: failed to get wrbpod slot {} for app '{}' in contract '{}': {:?}", app_slot_num, &app_name, &contract_addr, &e);
                process::exit(1);
            })
            .unwrap();
        ...
        if argv.len() < 7 {
            eprintln!("Usage: {} wrbpod {} [-w wrbpod_contract_addr] APP_NAME APP_SLOT_NUM APP_SLICE_ID", &argv[0], &cmd);
            process::exit(1);
        }
        let contract_addr = get_wrbpod_contract_id(&mut argv)
            .map_err(|e| {
        ...
        let app_slice_id: u128 = argv[5].parse()
            .map_err(|_e| {
        ...
        with_globals(|globals| {
            ...
            wrbpod_session.sync_slot(&app_name, app_slot_num)
                .map_err(|e| {
...
fn main() {
    ...
    if fs::metadata(&conf_path).is_err() {
        ...
        fs::write(&conf_path, default_conf_str)
            .map_err(|e| {
                eprintln!("FATAL: failed to write default config to '{}': {:?}", &conf_path, &e);
                process::exit(1);
            })
            .unwrap()
    }
    ...
    let conf = Config::from_path(&conf_path)
        .map_err(|e| {
            usage(&format!("Could not load config from '{}': {}", &conf_path, &e));
            unreachable!()
        })
        .unwrap();
...
mod tests {
    ...
    fn test_json_to_clarity() {
        ...
        let val_d = val_c_tuple.get("d").cloned().unwrap().expect_bool().unwrap();
        assert_eq!(val_d, false);

        let val_e = val_c_tuple.get("e").cloned().unwrap().expect_optional().unwrap();
        assert_eq!(val_e, None);

        let val_f = val_c_tuple.get("f").cloned().unwrap().expect_list().unwrap();
    ...
    fn test_wrbpod_format() {
        ...
        let contract_addr = QualifiedContractIdentifier::parse("SP1B62RVBBP8N4K3X4K6AA8FFPXQWGGX48SSEKPAB.wrbpod").unwrap();
        let args = vec![
            "wrb-test".to_string(),
            "wrbpod".to_string(),
            "format".to_string(),
    ...
    fn test_wrbpod_get_put_slot() {
        ...
        let mut slot = with_globals(|globals| {
            let wrbpod_session = globals.get_wrbpod_session(0).unwrap();
            wrbpod_session.fetch_chunk("hello-get-put-slot.btc", 0)
                .map_err(|e| {
                    panic!("FATAL: failed to fetch chunk: {:?}", &e);
                })
                .unwrap();

            let chunk_ref = wrbpod_session.ref_chunk(1)
                .unwrap();
